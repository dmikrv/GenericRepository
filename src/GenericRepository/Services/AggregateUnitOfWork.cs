...
/// <inheritdoc />
/// <remarks>
///     Be aware there is no distributed lock mechanism here. All units of work are saved sequentially, so you need to
///     provide your own mechanism for rolling back changes if some of units were committed by the time error occured.
/// </remarks>
public class AggregateUnitOfWork : IAggregateUnitOfWork
{
    ...
    {
        foreach (var unitOfWork in _unitsOfWork) await unitOfWork.SaveChangesAsync(token);
    }

    /// <inheritdoc />
    public void Rollback()
    {
        foreach (var unitOfWork in _unitsOfWork) unitOfWork.Rollback();
    }
}